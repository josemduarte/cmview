...
public class Start {
	...
	public static final String		VERSION = 				"0.1.2";			// current version of this application (should match manifest)
	...
	public static void main(String args[]){
		...
		try {
			...
			System.out.println("No configuration file found in current directory.");
		} catch (IOException e) {
			System.err.println("Error reading file ./" + CONFIG_FILE_NAME + ".");
		}
		// search in user home directory
		File userConfigFile = new File(System.getProperty("user.home"),CONFIG_FILE_NAME);
		try {
			if (userConfigFile.exists()) {
				System.out.println("Loading configuration file " + userConfigFile.getAbsolutePath());
				userProperties.putAll(loadUserProperties(userConfigFile.getAbsolutePath()));
				applyUserProperties(userProperties);
				configFileFound = true;
			}
		} catch (FileNotFoundException e) {
			System.out.println("No configuration file found in home directory.");
		} catch (IOException e) {
			System.err.println("Error while reading from file " + userConfigFile.getAbsolutePath() + ".");
		}
		// search in location given by command line parameter
		try{
			...
			System.out.println("Configuration file " + cmdLineConfigFile + " not found.");
		} catch (IOException e) {
			System.err.println("Error while reading from file " + cmdLineConfigFile + ".");
		}
		if(!configFileFound) {
			System.out.println("No configuration file found. Using default setting.");
		}
